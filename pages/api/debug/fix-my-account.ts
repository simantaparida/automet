...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!supabaseAdmin) {
        return res.status(500).json({ error: 'Server configuration error', message: 'Supabase Admin client not available' });
    }
    ...
    if (orgError) {
        return res.status(500).json({ error: 'Failed to upsert organization', details: orgError });
    }
    ...
    if (userError) {
        return res.status(500).json({ error: 'Failed to upsert user', details: userError });
    }
