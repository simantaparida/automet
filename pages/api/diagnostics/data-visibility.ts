    ...
    try {
        ...
        if (supabaseAdmin) {
            const [clients, sites, assets, jobs, inventory] = await Promise.all([
                supabaseAdmin.from('clients').select('id, name').eq('org_id', user.org_id!),
                supabaseAdmin.from('sites').select('id, name').eq('org_id', user.org_id!),
                supabaseAdmin.from('assets').select('id, asset_type').eq('org_id', user.org_id!),
                supabaseAdmin.from('jobs').select('id, title, status').eq('org_id', user.org_id!),
                supabaseAdmin.from('inventory_items').select('id, name').eq('org_id', user.org_id!),
            ]);

            results.dataInDatabase = {
                clients: { count: clients.data?.length || 0, sample: clients.data?.slice(0, 2) },
                sites: { count: sites.data?.length || 0, sample: sites.data?.slice(0, 2) },
                assets: { count: assets.data?.length || 0, sample: assets.data?.slice(0, 2) },
                jobs: { count: jobs.data?.length || 0, sample: jobs.data?.slice(0, 2) },
                inventory: { count: inventory.data?.length || 0, sample: inventory.data?.slice(0, 2) },
            };
        }

        // Check what user can see via RLS (authenticated access)
        const [clientsRLS, sitesRLS, assetsRLS, jobsRLS, inventoryRLS] = await Promise.all([
        ...
        results.comparison = {
            clients: {
                ...
                match: results.dataInDatabase.clients?.count === results.dataViaRLS.clients?.count,
            },
            sites: {
                inDB: results.dataInDatabase.sites?.count || 0,
                viaRLS: results.dataViaRLS.sites?.count || 0,
                match: results.dataInDatabase.sites?.count === results.dataViaRLS.sites?.count,
            },
            assets: {
                inDB: results.dataInDatabase.assets?.count || 0,
                viaRLS: results.dataViaRLS.assets?.count || 0,
                match: results.dataInDatabase.assets?.count === results.dataViaRLS.assets?.count,
            },
            jobs: {
                inDB: results.dataInDatabase.jobs?.count || 0,
                viaRLS: results.dataViaRLS.jobs?.count || 0,
                match: results.dataInDatabase.jobs?.count === results.dataViaRLS.jobs?.count,
            },
            inventory: {
                inDB: results.dataInDatabase.inventory?.count || 0,
                viaRLS: results.dataViaRLS.inventory?.count || 0,
                match: results.dataInDatabase.inventory?.count === results.dataViaRLS.inventory?.count,
            },
        };

        results.diagnosis = {
            hasDataInDB: Object.values(results.dataInDatabase).some((d: any) => d.count > 0),
            canAccessViaRLS: Object.values(results.dataViaRLS).some((d: any) => d.count > 0),
            rlsIssue: Object.values(results.comparison).some((c: any) => !c.match && c.inDB > 0),
        };
